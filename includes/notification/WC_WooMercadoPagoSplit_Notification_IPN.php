...
class WC_WooMercadoPagoSplit_Notification_IPN extends WC_WooMercadoPagoSplit_Notification_Abstract
{
    ...
    public function check_ipn_response()
    {
        ...
        if (!isset($data['id']) || !isset($data['topic'])) {
            ...
            $this->setResponse(422, null, __('No ID or TOPIC param in Request IPN', 'woocommerce-mercadopago-split'));
        }
        ...
        if ($data['topic'] == 'payment' || $data['topic'] != 'merchant_order') {
            ...
            $this->setResponse(422, null, __('Type of topic IPN invalid, need to be merchant_order', 'woocommerce-mercadopago-split'));
        }
        ...
        if ($data['topic'] == 'merchant_order') {
            ...
            if (is_wp_error($ipn_info) || ($ipn_info['status'] != 200 && $ipn_info['status'] != 201)) {
                ...
                $this->setResponse(422, null, __('IPN merchant_order not found', 'woocommerce-mercadopago-split'));
            }

            $payments = $ipn_info['response']['payments'];
            if (sizeof($payments) < 1) {
                $this->log->write_log(__FUNCTION__, 'Not found Payments into Merchant_Order');
                $this->setResponse(422, null, __('Not found Payments into Merchant_Order', 'woocommerce-mercadopago-split'));
            }
            ...
            if (!empty($data['payments'])) {
                ...
                foreach ($data['payments'] as $payment) {
                    ...
                    $order->update_meta_data(
                        'Mercado Pago Split - Payment ' . $payment['id'],
