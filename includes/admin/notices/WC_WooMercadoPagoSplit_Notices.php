...
/**
 * Class WC_WooMercadoPagoSplit_Notification
 */
class WC_WooMercadoPagoSplit_Notices
{
    ...
    public static function getAlertFrame($message, $type)
    {
        $inline = null;
        if (
            (class_exists('WC_WooMercadoPagoSplit_Module') && WC_WooMercadoPagoSplit_Module::isWcNewVersion())
            &&
        ...
        $notice = '<div id="message" class="notice ' . $type . ' is-dismissible ' . $inline . '">
                    ...
                    <button type="button" class="notice-dismiss">
                        <span class="screen-reader-text">' . __('Discard', 'woocommerce-mercadopago-split') . '</span>
                    </button>
                </div>';
    ...
    public static function getAlertWocommerceMiss($message, $type)
    {
        ...
        if ($is_installed && current_user_can('install_plugins')) {
            $buttonUrl = '<a href="' . wp_nonce_url(self_admin_url('plugins.php?action=activate&plugin=woocommerce/woocommerce.php&plugin_status=active'), 'activate-plugin_woocommerce/woocommerce.php') . '" class="button button-primary">' . __('Activate WooCommerce', 'woocommerce-mercadopago-split') . '</a>';
        } else {
            if (current_user_can('install_plugins')) {
                $buttonUrl = '<a href="' . wp_nonce_url(self_admin_url('update.php?action=install-plugin&plugin=woocommerce'), 'install-plugin_woocommerce') . '" class="button button-primary">' . __('Install WooCommerce', 'woocommerce-mercadopago-split') . '</a>';
            } else {
                $buttonUrl = '<a href="http://wordpress.org/plugins/woocommerce/" class="button button-primary">' . __('See WooCommerce', 'woocommerce-mercadopago-split') . '</a>';
            }
        }
