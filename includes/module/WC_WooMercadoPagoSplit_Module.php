...
class WC_WooMercadoPagoSplit_Module extends WC_WooMercadoPagoSplit_Configs
{
    ...
    public function __construct()
    {
        try {
            ...
            add_filter('plugin_action_links_' . WC_MERCADOPAGO_SPLT_BASENAME, array($this, 'woomercadopago_settings_link'));
    ...
    public static function getMpInstance($payment = null)
    {
        ...
        if ($validateCredentialsType == WC_WooMercadoPagoSplit_Credentials::TYPE_ACCESS_CLIENT) {
    ...
    /**
     * @return WC_WooMercadoPagoSplit_Module|null
     * Singleton
     */
    ...
    public function loadPayments()
    {
        ...
        // include_once dirname(__FILE__) . '/../payments/WC_WooMercadoPagoSplit_CustomGateway.php';
        // include_once dirname(__FILE__) . '/../payments/WC_WooMercadoPagoSplit_TicketGateway.php';
    ...
    public function woomercadopago_settings_link($links)
    {
        ...
        $plugin_links = array();
        $plugin_links[] = '<a href="' . admin_url('admin.php?page=wc-settings&tab=checkout') . '">' . __('Set up', 'woocommerce-mercadopago-split') . '</a>';
        // $plugin_links[] = '<a target="_blank" href="' . 'https://wordpress.org/support/plugin/woocommerce-mercadopago-split/reviews/?rate=5#new-post' . '">' . __('Your opinion helps us get better', 'woocommerce-mercadopago-split') . '</a>';
        // $plugin_links[] = '<br><a target="_blank" href="' . 'https://www.mercadopago.' . $links_mp['sufix_url'] . 'developers/' . $links_mp['translate'] . '/guides/plugins/woocommerce/introduction/' . '">' . __('Guides and Documentation', 'woocommerce-mercadopago-split') . '</a>';
        // $plugin_links[] = '<a target="_blank" href="' . 'https://www.mercadopago.' . $links_mp['sufix_url'] . $links_mp['help'] . '">' . __('Report Problem', 'woocommerce-mercadopago-split') . '</a>';
    ...
    public function mp_plugin_row_meta($links, $file)
    {
        if (WC_MERCADOPAGO_SPLT_BASENAME === $file) {
            $new_link = array();
            $new_link[] = $links[0];
            $new_link[] = esc_html__('By Mercado Pago', 'woocommerce-mercadopago-split');
    ...
    public static function get_common_error_messages($key)
    {
        if ($key === 'Invalid payment_method_id') {
            return __('The payment method is not valid or not available.', 'woocommerce-mercadopago-split');
        }
        if ($key === 'Invalid transaction_amount') {
            return __('The transaction amount cannot be processed by Mercado Pago.', 'woocommerce-mercadopago-split') . ' ' .
        ...
        if ($key === 'Invalid users involved') {
            return __('The users are not valid.', 'woocommerce-mercadopago-split') . ' ' .
        ...
        if ($key === 'Unauthorized use of live credentials') {
            return __('Unauthorized use of production credentials.', 'woocommerce-mercadopago-split') . ' ' .
    ...
    {
        $site_data = WC_WooMercadoPagoSplit_Module::get_site_data();
    ...
    {
        return get_woocommerce_currency() == WC_WooMercadoPagoSplit_Module::$country_configs[$site_id]['currency'];
    }
    ...
    public static function build_currency_conversion_err_msg($currency)
    {
        return '<img width="14" height="14" src="' .
            ...
            __('a', 'woocommerce-mercadopago-split') . ' ' . $currency . '. ' .
    ...
    public static function get_country_name($site_id)
    {
        switch ($site_id) {
            case 'MCO':
                return __('Colombia', 'woocommerce-mercadopago-split');
            case 'MLA':
                return __('Argentina', 'woocommerce-mercadopago-split');
            case 'MLB':
                return __('Brazil', 'woocommerce-mercadopago-split');
            case 'MLC':
                return __('Chile', 'woocommerce-mercadopago-split');
            case 'MLM':
                return __('Mexico', 'woocommerce-mercadopago-split');
            case 'MLU':
                return __('Uruguay', 'woocommerce-mercadopago-split');
            case 'MLV':
                return __('Venezuela', 'woocommerce-mercadopago-split');
            case 'MPE':
                return __('Peru', 'woocommerce-mercadopago-split');
        }
        return '';
    }
