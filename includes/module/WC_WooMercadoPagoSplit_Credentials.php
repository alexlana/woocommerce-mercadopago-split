...
class WC_WooMercadoPagoSplit_Credentials
{
    ...
    public function __construct($payment = null)
    {
        ...
        if ( (int)$vendedor > 0 ) {
            $credenciais = json_decode( get_user_meta( $vendedor, 'wcmps_credentials', true ) );
            $publicKey = $credenciais->public_key;
    ...
    {
        $mp_v1 = WC_WooMercadoPagoSplit_Module::getMpInstanceSingleton();
        if (empty($mp_v1)) {
            return false;
        }
