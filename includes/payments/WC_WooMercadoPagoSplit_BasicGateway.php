...
/**
 *
 * WC_WooMercadoPagoSplit_BasicGateway
 *
 */
class WC_WooMercadoPagoSplit_BasicGateway extends WC_WooMercadoPagoSplit_PaymentAbstract
{
    ...
    public function is_available()
    {
        ...
        if (isset($this->settings['enabled']) && $this->settings['enabled'] == 'yes') {
            if ($this->mp instanceof MP) {
                ...
                if (WC_WooMercadoPagoSplit_Credentials::validateCredentialsTest($this->mp ,$accessToken) == false && $this->sandbox == true) {
                    return false;
                }

                if (WC_WooMercadoPagoSplit_Credentials::validateCredentialsProd($this->mp ,$accessToken) == false && $this->sandbox == false) {
                    return false;
                }
                return true;
            }
    ...
    public function field_checkout_header()
    {
        $checkout_header = array(
            'title' => sprintf(
                ...
                ' . __('Accept all method of payment and take your charges to another level', 'woocommerce-mercadopago-split') . '
                 </div>
              <div class="mp-col-md-12">
                <p class="mp-text-checkout-body mp-mb-0">
                  ' . __('Turn your online store into your customers preferred payment gateway. Choose if the final payment experience will be inside or outside your store.', 'woocommerce-mercadopago-split') . '
                </p>
              </div>
            </div>'
            ),
    ...
    public function field_success_url()
    {
        ...
        if (!empty($this->success_url) && filter_var($this->success_url, FILTER_VALIDATE_URL) === FALSE) {
            $success_back_url_message = '<img width="14" height="14" src="' . plugins_url('assets/images/warning.png', plugin_dir_path(__FILE__)) . '"> ' .
                __('This seems to be an invalid URL.', 'woocommerce-mercadopago-split') . ' ';
        } else {
            $success_back_url_message = __('Choose the URL that we will show your customers when they finish their purchase.', 'woocommerce-mercadopago-split');
        }
        $success_url = array(
    ...
    public function field_failure_url()
    {
        if (!empty($this->failure_url) && filter_var($this->failure_url, FILTER_VALIDATE_URL) === FALSE) {
            ...
            $fail_back_url_message = __('Choose the URL that we will show to your customers when we refuse their purchase. Make sure it includes a message appropriate to the situation and give them useful information so they can solve it.', 'woocommerce-mercadopago-split');
        }
        $failure_url = array(
    ...
    public function field_pending_url()
    {
        ...
        if (!empty($this->pending_url) && filter_var($this->pending_url, FILTER_VALIDATE_URL) === FALSE) {
            ...
            $pending_back_url_message = __('Choose the URL that we will show to your customers when they have a payment pending approval.', 'woocommerce-mercadopago-split');
        }
        $pending_url = array(
                ...
                if ($payment_method['type'] == 'credit_card') {
                    $element = array(
                        ...
                        'custom_attributes' => array(
                            'data-translate' => __('Select credit cards', 'woocommerce-mercadopago-split')
                        ),
                    );
                    ...
                    $element = array(
                        ...
                        'custom_attributes' => array(
                            'data-translate' => __('Select debit cards', 'woocommerce-mercadopago-split')
                        ),
                    );
                } else {
                    $element = array(
                        ...
                        'custom_attributes' => array(
                            'data-translate' => __('Select offline payments', 'woocommerce-mercadopago-split')
                        ),
                    );
                }
            }
...
// exit;
    ...
    public static function getId(){
        return WC_WooMercadoPagoSplit_BasicGateway::ID;
    }
}
