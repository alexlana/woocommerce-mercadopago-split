...
abstract class WC_WooMercadoPagoSplit_Hook_Abstract
{
    ...
    {
        if (!preg_match('/woo-mercado-pago-split/', $id)) {
            return $title;
        }
    ...
    private function validatePublicKey($key, $value)
    {
        ...
        if($key == '_mp_public_key_prod' && WC_WooMercadoPagoSplit_Credentials::validateCredentialsProd($this->mpInstance, null ,$value) == false) {
            update_option($key, '', true);
            add_action('admin_notices', array($this, 'noticeInvalidPublicKeyProd'));
            return true;
        }

        if($key == '_mp_public_key_test' && WC_WooMercadoPagoSplit_Credentials::validateCredentialsTest($this->mpInstance, null ,$value) == false) {
            update_option($key, '', true);
    ...
    private function validateAccessToken($key, $value, $isProduction = null)
    {
        ...
        if ($key == '_mp_access_token_prod' && WC_WooMercadoPagoSplit_Credentials::validateCredentialsProd($this->mpInstance, $value, null) == false) {
            add_action('admin_notices', array($this, 'noticeInvalidProdCredentials'));
            update_option($key, '', true);
            return true;
        }

        if ($key == '_mp_access_token_test' && WC_WooMercadoPagoSplit_Credentials::validateCredentialsTest($this->mpInstance, $value, null) == false) {
